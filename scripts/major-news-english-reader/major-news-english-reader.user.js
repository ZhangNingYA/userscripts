// ==UserScript==
// @name         Major News English Reader
...
(function () {
    ...
    function safeGetValue(key, fallback) {
        try {
            ...
            console.warn('[Major News English Reader] Failed to read storage', error);
            return fallback;
        }
    }
    ...
    function safeSetValue(key, value) {
        try {
            ...
            console.warn('[Major News English Reader] Failed to save storage', error);
        }
    }
    ...
    function buildToolbar() {
        ...
        toolbarRoot.setAttribute('aria-label', 'Major News English Reader');
    ...
    function registerMenu() {
        ...
        GM_registerMenuCommand('Major News Reader: 设置', showSettings);
        GM_registerMenuCommand('Major News Reader: 继续精读', () => analyzeSentences());
    }
    ...
    function showSettings() {
        ...
        panel.setAttribute('aria-label', 'Major News English Reader settings');
        panel.innerHTML = `
            <div class="rer-settings-header">
                <span class="rer-settings-mark" aria-hidden="true">N</span>
                <div class="rer-settings-title">
                    <p class="rer-settings-eyebrow">MAJOR NEWS READER</p>
    ...
    function insertSentenceControls(item, textMap, placeDetailOutside) {
        ...
        for (const item of getConnectedReadingItems()) {
            ...
            for (const span of item.analysis.spans) {
                ...
                try {
                    ...
                    console.warn('[Major News English Reader] Failed to map structure highlight', error);
                }
            }
        }
    ...
    async function analyzeSentences(options = {}) {
        ...
        const worker = async () => {
            while (nextBatch < batches.length) {
                ...
                try {
                    ...
                    console.warn('[Major News English Reader] Analysis batch failed', error);
                }
                updateLoadedCount();
            }
        };
        try {
            ...
            if (runGeneration === analysisGeneration) saveAnalysisCache();
            if (failures.size) console.info(`[Major News English Reader] ${failures.size} sentence(s) remain unloaded.`);
        } finally {
            for (const item of pending) {
                item.loading = false;
                if (!item.ready) {
    ...
    async function analyzeBatch(batch) {
        ...
        const response = await requestChat({
            system: [
                'You create concise Chinese reading notes for English news sentences.',
            ...
            user: [
                ...
                'Return an array like [{"id":"mner-s-1","translation":"...","phrases":[{"text":"...","meaning":"..."}],"pattern":"SVO","spans":[{"text":"Officials","start":0,"end":9,"occurrence":1,"role":"subject"}]}].',
    ...
    function applyAnalysisResult(result, persist) {
        ...
        try {
            ...
            console.warn('[Major News English Reader] Failed to render sentence analysis', error);
            return false;
        }
